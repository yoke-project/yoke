...
# std: yoke:verbs-and-licence.02
check_no_path_outside() {
  [[ -f "$root/justfile" ]] || { echo "no justfile"; return 1; }
  local hits
  hits="$(cd "$root" && grep -nE '(^|[^.])\.\./' justfile go.mod go.work 2>/dev/null)"
  [[ -z "$hits" ]] || { echo "a path outside the repository: $hits"; return 1; }
}

# std: yoke:verbs-and-licence.03
check_container_build() {
  ...
  "$engine" build -q -t yoke-check-build -f "$root/Containerfile" "$root" >/dev/null \
    || { echo "the image did not build"; return 1; }
  "$engine" run --rm yoke-check-build just build >/dev/null \
    || { echo "build failed inside the container"; return 1; }
}

# std: yoke:verbs-and-licence.04
check_licence() {
