...
| **Expected** | one JSON record naming this repository, the commit it was run at, the level, the tier, the environment the run fixed and the artifacts it involved, with an entry for every case declared at that level and `state` passed — assembled from what the run left, and never from a second run |

## yoke:the-record-of-a-run.03 — the record of a failing run says so, and blocks
...
## yoke:the-record-of-a-run.04 — the workflow hands the record over, and can fail nothing
...
## yoke:the-record-of-a-run.05 — nothing a run writes enters the tree
...
| **Action** | ask git what changed in the tree |
| **Expected** | nothing — the results and the record are ignored, so a contributor who runs the verbs has nothing to undo and nothing to commit by mistake |
