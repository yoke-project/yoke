...
| **Planning item** | yoke-project/yoke#29 |

## yoke:planning-status.01 — the workflow runs on the work, and runs nothing of its own
...
## yoke:planning-status.02 — it is granted nothing, and with no credential it does nothing
...
## yoke:planning-status.03 — an item moves forward, and never back
...
## yoke:planning-status.05 — a query that fails moves nothing, and says so
