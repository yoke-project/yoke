...
check_granted_nothing_and_silent_without_a_credential() {
  ...
  local out status
  out="$(GH_TOKEN="" GITHUB_REPOSITORY=yoke-project/yoke "$planning_script" push 2>&1)"
  status=$?
...
JSON
  named="$(items_named_by_push "$event" "yoke-project/yoke")"
  rm -rf "$tmp"

  local want=$'yoke-project/meta-yoke#7\nyoke-project/yoke#29'
  [[ "$(sort <<<"$named")" == "$want" ]] || { echo "it read: $(tr '\n' ' ' <<<"$named")"; return 1; }
}

# std: yoke:planning-status.05
...
SH
  ...
  out="$(PATH="$tmp:$PATH" GH_TOKEN=a-credential PLANNING_ORG=yoke-project PLANNING_PROJECT=1 \
    GITHUB_REPOSITORY=yoke-project/yoke GITHUB_EVENT_PATH="$tmp/event.json" \
